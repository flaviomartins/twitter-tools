...
class IndexThreads {
  ...
  private static class IndexThread extends Thread {
    ...
    private Document getDocumentFromDocData(Status status) {
      ...
      doc.add(new LongPoint(IndexStatuses.StatusField.ID.name, id));
      doc.add(new StoredField(IndexStatuses.StatusField.ID.name, id));

      doc.add(new LongPoint(IndexStatuses.StatusField.EPOCH.name, status.getCreatedAt().getTime() / 1000L));
      doc.add(new StoredField(IndexStatuses.StatusField.EPOCH.name, status.getCreatedAt().getTime() / 1000L));
      ...
      doc.add(new IntPoint(IndexStatuses.StatusField.FRIENDS_COUNT.name, status.getUser().getFriendsCount()));
      doc.add(new StoredField(IndexStatuses.StatusField.FRIENDS_COUNT.name, status.getUser().getFriendsCount()));

      doc.add(new IntPoint(IndexStatuses.StatusField.FOLLOWERS_COUNT.name, status.getUser().getFollowersCount()));
      doc.add(new StoredField(IndexStatuses.StatusField.FOLLOWERS_COUNT.name, status.getUser().getFollowersCount()));

      doc.add(new IntPoint(IndexStatuses.StatusField.STATUSES_COUNT.name, status.getUser().getStatusesCount()));
      doc.add(new StoredField(IndexStatuses.StatusField.STATUSES_COUNT.name, status.getUser().getStatusesCount()));

      long inReplyToStatusId = status.getInReplyToStatusId();
      if (inReplyToStatusId > 0) {
        doc.add(new LongPoint(IndexStatuses.StatusField.IN_REPLY_TO_STATUS_ID.name, inReplyToStatusId));
        doc.add(new StoredField(IndexStatuses.StatusField.IN_REPLY_TO_STATUS_ID.name, inReplyToStatusId));

        doc.add(new LongPoint(IndexStatuses.StatusField.IN_REPLY_TO_USER_ID.name, status.getInReplyToUserId()));
        doc.add(new StoredField(IndexStatuses.StatusField.IN_REPLY_TO_STATUS_ID.name, inReplyToStatusId));
      }
      ...
      if (retweetCount > -1) {
        doc.add(new IntPoint(IndexStatuses.StatusField.RETWEET_COUNT.name, retweetCount));
        doc.add(new StoredField(IndexStatuses.StatusField.RETWEET_COUNT.name, retweetCount));
      }

      Status retweetedStatus = status.getRetweetedStatus();
      if (retweetedStatus != null) {
        doc.add(new LongPoint(IndexStatuses.StatusField.RETWEETED_STATUS_ID.name, retweetedStatus.getId()));
        doc.add(new StoredField(IndexStatuses.StatusField.RETWEETED_STATUS_ID.name, retweetedStatus.getId()));

        doc.add(new LongPoint(IndexStatuses.StatusField.RETWEETED_USER_ID.name, retweetedStatus.getUser().getId()));
        doc.add(new StoredField(IndexStatuses.StatusField.RETWEETED_USER_ID.name, retweetedStatus.getUser().getId()));
      }
      return doc;
    }
